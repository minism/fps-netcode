...
public class CameraController : MonoBehaviour {
  [Header("Target")] public Transform followTarget;
  public float targetEyeHeight;
  public float targetEyeDepth;
  public float followSharpness = 10000f;

  [Header("Rotation params")] public float minTilt = -90f;
  ...
  private void Update() {
    ...
    var axisRight = Input.GetAxisRaw("Mouse X");
    var axisUp = Input.GetAxisRaw("Mouse Y");
  ...
  public void ApplyLookInput(Vector3 lookInputVector, float dt) {
    // Process yaw.
    var rotFromInput = Quaternion.Euler(Vector3.up * (lookInputVector.x * rotationSpeed));
    ...
    // Lerp the rotation.
    var yawRot = Quaternion.LookRotation(targetYawDirection, followTarget.up);
    var tiltRot = Quaternion.Euler(targetTilt, 0, 0);
    // TODO: Apply this optimization here (two quats at once) to StarArk code.
    var targetRot = Quaternion.Slerp(
